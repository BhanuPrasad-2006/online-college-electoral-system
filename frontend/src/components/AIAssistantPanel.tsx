...
export function AIAssistantPanel({ compact = false }: { compact?: boolean }) {
  const [msgs, setMsgs] = useState<Msg[]>([
    {
      from: "ai",
      text: "Hi! I can help you compare candidates and explore manifestos. Ask me anything.",
    },
  ]);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Neutral candidate research and comparison.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {SUGGESTIONS.map((s) => (
          <button
            key={s}
            onClick={() => send(s)}
            className="px-3 py-2 rounded-full bg-muted hover:bg-muted/70 text-xs flex items-center gap-1.5"
          >
      ...
      <section
        className={`bg-card rounded-2xl shadow-sm flex flex-col ${compact ? "h-[calc(100vh-260px)] min-h-[420px]" : "h-[520px]"}`}
      >
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {msgs.map((m, i) => (
            <div key={i} className={`flex ${m.from === "user" ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm ${m.from === "user" ? "bg-[#1F3A6E] text-white rounded-br-sm" : "bg-muted text-foreground rounded-bl-sm"}`}
              >
                <p>{m.text}</p>
