...
import { Upload, QrCode, CheckCircle2, AlertCircle, Lock, Mail, User, FileText, Award, Phone } from "lucide-react";
import { cn } from "@/lib/utils";
import { toast } from "sonner";
import { checkCandidateEligibility, verifyEligibilityOtp, fetchPositions, registerCandidate } from "@/lib/api";
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4 py-8">
      <div className="w-full max-w-3xl bg-card rounded-2xl shadow-sm border border-border/60 p-6 md:p-10 relative overflow-hidden glass-panel">
        ...
        <p className="text-sm text-muted-foreground">Complete the strict 7-stage registration process to apply.</p>
        ...
        <div className="flex items-center gap-2 mt-6 overflow-x-auto pb-3 border-b border-border/40 scrollbar-none">
          {STEPS.map((s, i) => (
            <div key={s} className="flex items-center shrink-0">
              ...
              {i < STEPS.length - 1 && <div className={cn("w-4 h-[2px] mx-1", i < step ? "bg-success/40" : "bg-border/60")} />}
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 min-h-[300px]">
          ...
          {step === 1 && (
            <div className="space-y-5 max-w-md mx-auto text-center">
              ...
              <p className="text-xs text-muted-foreground">
                We've sent a 6-digit confirmation code to <span className="font-semibold text-foreground">{email}</span>.
              </p>
          ...
          {step === 2 && (
            <div className="space-y-6 max-w-md mx-auto">
              <div className="text-center mb-4">
                ...
                <p className="text-xs text-muted-foreground">Select the official council position you wish to contest for.</p>
              </div>
              ...
              {selectedPositionId && (
                <div className="bg-muted/40 border border-border/40 p-4 rounded-xl text-xs space-y-2">
                  ...
                  <p className="text-muted-foreground leading-relaxed">
                    {positions.find(p => p.position_id === selectedPositionId)?.description || "No official description available."}
                  </p>
                </div>
              )}
          ...
          {step === 3 && (
            <div className="space-y-5 max-w-md mx-auto">
              <div className="text-center mb-4">
                ...
                <p className="text-xs text-muted-foreground">These details are synced directly from the voter database.</p>
              </div>

              <div className="space-y-3.5">
                <Field label="Full Name"><Input value={formData.name} disabled className="bg-muted/50" /></Field>
                <Field label="Department"><Input value={formData.department} disabled className="bg-muted/50" /></Field>
                <Field label="Semester/Year"><Input value={formData.semester} disabled className="bg-muted/50" /></Field>
          ...
          {step === 4 && (
            <div className="space-y-5 max-w-lg mx-auto">
              <div className="text-center mb-3">
                ...
                <p className="text-xs text-muted-foreground">Provide details about your student coalition group and political statement.</p>
              </div>
          ...
          {step === 5 && (
            <div className="text-center space-y-5 max-w-md mx-auto">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                As per Election Committee guidelines, all candidates must pay a one-time refundable registration fee of <span className="font-bold text-foreground">₹500</span>.
              </p>
              ...
              <UploadBox 
                ...
                onSet={(v) => setFormValue("paymentScreenshotUrl", "https://api.dicebear.com/7.x/identicon/svg?seed=" + v)}
              />
          ...
          {step === 6 && (
            <div className="space-y-5">
              <h3 className="text-lg font-semibold border-b border-border/40 pb-2">Final Application Review</h3>
              ...
              <div className="bg-muted/40 p-4 rounded-xl text-xs space-y-2">
                ...
                <p className="text-muted-foreground italic leading-relaxed">"{formData.manifesto}"</p>
              </div>
              ...
              <label className="flex items-start gap-3 mt-5 cursor-pointer bg-muted/20 p-3 rounded-xl border border-border/40 select-none">
                ...
                <div className="space-y-0.5">
                  <p className="text-sm font-semibold text-foreground">Acknowledge Rules & Guidelines</p>
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    I verify all information is accurate and declare my eligibility. I agree to comply with all guidelines set by the Election Committee.
                  </p>
                </div>
              </label>
