      ...
      <div className="flex flex-wrap gap-2">
        {QUICK_ACTIONS.map((action) => (
          <button
            ...
            disabled={loading}
            className="px-3 py-2 rounded-xl bg-gradient-to-r from-[#0F8A5F]/10 to-[#0F8A5F]/10 hover:from-[#0F8A5F]/20 hover:to-[#0F8A5F]/20 border border-[#D9A441]/20 text-xs font-medium flex items-center gap-1.5 transition-all disabled:opacity-50"
          >
            <span className="text-[#0F8A5F]">{action.icon}</span>
            {action.label}
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {SUGGESTIONS.map((s) => (
          <button
            ...
            onClick={() => send(s)}
            disabled={loading}
            className="px-3 py-2 rounded-full bg-muted hover:bg-[#0F8A5F]/10 hover:text-[#0F8A5F] border border-border text-xs flex items-center gap-1.5 transition-colors disabled:opacity-50"
          >
            <Sparkles className="h-3 w-3 text-[#0F8A5F]" /> {s}
          </button>
        ))}
      </div>
  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          {/* ── Header ── */}
          <div className="bg-gradient-to-r from-[#1a1a2e] via-[#0F8A5F] to-[#0F8A5F] px-4 py-3 flex items-center gap-3 shrink-0">
            <div className="relative shrink-0">
              ...
              <span className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full bg-emerald-400 border-2 border-[#0F8A5F]" />
            </div>
            ...
            <>
                    ...
                    <div className="flex flex-wrap gap-1.5 pb-1">
                      {QUICK_ACTIONS.map((action) => (
                        <button
                          ...
                          onClick={() => send(action.prompt)}
                          disabled={loading}
                          className="px-2.5 py-1.5 rounded-xl bg-gradient-to-r from-[#0F8A5F]/10 to-[#0F8A5F]/10 border border-[#D9A441]/20 text-[10px] font-medium text-[#0F8A5F] hover:from-[#0F8A5F]/20 hover:to-[#0F8A5F]/20 transition-all disabled:opacity-40 flex items-center gap-1"
                        >
                    ...
                    <div className="flex flex-wrap gap-1.5 pb-1">
                      {suggestions.map((s) => (
                        <button
                          ...
                          onClick={() => send(s)}
                          disabled={loading}
                          className="px-2.5 py-1.5 rounded-full bg-white dark:bg-white/5 border border-[#0F8A5F]/25 text-[#0F8A5F] text-[10px] font-medium hover:bg-[#0F8A5F] hover:text-white hover:border-[#0F8A5F] transition-all duration-150 disabled:opacity-40 shadow-sm flex items-center gap-1"
                        >
              ...
              <div className="bg-white dark:bg-[#0F1724] border-t border-gray-100 dark:border-white/10 p-3 flex gap-2 items-center shrink-0">
                <input
                  ...
                  disabled={loading}
                  className="flex-1 h-9 px-3 rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-[#0F8A5F]/30 focus:border-[#0F8A5F]/60 disabled:opacity-50 transition-all"
                />
                <button
                  onClick={() => send(input)}
                  disabled={loading || !input.trim()}
                  className="h-9 w-9 rounded-xl bg-gradient-to-br from-[#1F2937] to-[#0F8A5F] text-white flex items-center justify-center hover:opacity-90 disabled:opacity-30 transition-all shrink-0 shadow-md shadow-[#0F8A5F]/30"
                >
                  {loading ? (
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-[9999] h-14 w-14 rounded-full bg-gradient-to-br from-[#1F2937] to-[#0F8A5F] text-white shadow-lg shadow-[#0F8A5F]/40 flex items-center justify-center transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-[#0F8A5F]/50 active:scale-95 group"
        aria-label="Toggle AI Assistant"
        title="Election AI Assistant"
      >
        <span
          className="absolute inset-0 rounded-full bg-[#0F8A5F]/30 animate-ping"
  ...
  return (
    <>
      {msgs.map((m, i) => (
        ...
        >
          {m.from === "ai" && (
            <div className="h-7 w-7 rounded-full bg-gradient-to-br from-[#0F8A5F]/20 to-[#0F8A5F]/20 flex items-center justify-center shrink-0 mt-0.5 border border-[#D9A441]/20">
              <Bot className="h-3.5 w-3.5 text-[#0F8A5F]" />
            </div>
          )}
          <div
          ...
          >
            <div
              className={`rounded-2xl px-3.5 py-2.5 text-[13px] leading-relaxed
                ${
                  m.from === "user"
                    ? "bg-gradient-to-br from-[#0F8A5F] to-[#4F46E5] text-white rounded-br-sm shadow-md shadow-[#0F8A5F]/25"
                    : m.isError
          ...
          {m.from === "user" && (
            <div className="h-7 w-7 rounded-full bg-[#0F8A5F]/10 dark:bg-white/10 flex items-center justify-center shrink-0 mt-0.5">
              <User className="h-3.5 w-3.5 text-[#0F8A5F] dark:text-white/70" />
            </div>
          )}
        </div>
      ))}
      ...
      {loading && (
        <div className="flex gap-2 justify-start animate-in fade-in duration-150">
          <div className="h-7 w-7 rounded-full bg-gradient-to-br from-[#0F8A5F]/20 to-[#0F8A5F]/20 flex items-center justify-center shrink-0 border border-[#D9A441]/20">
            <Bot className="h-3.5 w-3.5 text-[#0F8A5F]" />
          </div>
          <div className="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl rounded-bl-sm px-4 py-3 shadow-sm">
            <div className="flex items-center gap-1.5">
              {[0, 1, 2].map((i) => (
                <span
                  key={i}
                  className="h-1.5 w-1.5 rounded-full bg-[#0F8A5F]"
                  style={{
  ...
  return (
    <div className="border-t border-border p-3 flex gap-2 items-center bg-card rounded-b-2xl">
      <input
        ...
        disabled={loading}
        className="flex-1 h-10 px-3 rounded-xl border border-border bg-background text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-[#0F8A5F]/30 focus:border-[#0F8A5F]/50 disabled:opacity-50 transition-all"
      />
      <button
        onClick={() => send(input)}
        disabled={loading || !input.trim()}
        className="h-10 w-10 rounded-xl bg-gradient-to-br from-[#1F2937] to-[#0F8A5F] text-white flex items-center justify-center hover:opacity-90 disabled:opacity-30 transition-opacity shrink-0"
      >
