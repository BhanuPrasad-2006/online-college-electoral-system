          ...
          <tbody>
            {list.map((c) => (
              <tr key={c.candidate_id} className="border-b last:border-0 hover:bg-muted/30">
                ...
                <td className="p-4 text-xs max-w-[150px] truncate" title={c.admin_remarks || ""}>
                  {c.admin_remarks || "—"}
                </td>
                <td className="p-4">
                  <Badge
      ...
      <Dialog open={!!reject} onOpenChange={(b) => !b && setReject(null)}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">
            Provide a reason — this will be shown to the candidate.
          </p>
          <textarea
            value={reason}
          ...
          <div className="flex gap-2 justify-end">
            <Button variant="outline" onClick={() => setReject(null)} disabled={!!actionLoading}>
              Cancel
            </Button>
            <Button
