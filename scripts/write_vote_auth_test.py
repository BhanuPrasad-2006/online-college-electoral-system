"""Generate tests/backend/test_vote_auth.py — vote authentication E2E tests.
Fix: Use uuid.UUID objects in auth override to match what PostgreSQL expects."""
import pathlib

TEST_FILE = pathlib.Path("tests/backend/test_vote_auth.py")
...
print(f"Wrote {TEST_FILE.stat().st_size} bytes to {TEST_FILE}")
